...
Variable dictionary:
num = user input(int)
d = variable for end program(string)
x = starting loop(int)
...
print('Hello, welcome to photo organizer, answer the question to receive the dimensions, Enter the word done when you are done with the program')

print()

import math #math library
d = 'done' #Variable for end program
...
while(num != d): #Will continue to loop until the user input equals 'd'
    ...
    num = int(num)
    while(num <= 0): # will continue to loop if user keeps inputting the number<=0
